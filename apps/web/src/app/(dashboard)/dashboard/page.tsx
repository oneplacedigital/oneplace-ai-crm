...
export default function DashboardPage() {
  ...
  const { data: reps } = useSWR<RepStat[]>('/leads/counselor-stats', apiGet);

  const totalLeads = summary ? Object.values(summary).reduce((a, b) => a + b, 0) : 0;
  const won = summary?.[LeadStatus.WON] ?? 0;
  const qualifiedPlus =
    (summary?.[LeadStatus.QUALIFIED] ?? 0) +
    (summary?.[LeadStatus.PROPOSAL_SENT] ?? 0) +
    (summary?.[LeadStatus.NEGOTIATION] ?? 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <KPI label="In Progress" value={qualifiedPlus} accent="navy" />
        <KPI label="Won" value={won} accent="green" />
            ...
            <tbody className="divide-y divide-slate-100">
              {reps?.map((c) => (
                <tr key={c.counselorId}>
                  ...
                  <td className="py-3 pr-4">{c.byStatus.WON ?? 0}</td>
              ...
              {reps && reps.length === 0 && (
                <tr>
                  <td className="py-4 text-slate-500" colSpan={6}>
                    No team members yet. Add them under{' '}
                    <a className="text-brand" href="/counselors">Team</a>.
                  </td>
                </tr>
              )}
            </tbody>
