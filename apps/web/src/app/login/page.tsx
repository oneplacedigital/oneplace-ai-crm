      ...
      <section className="flex items-center justify-center bg-white p-6">
        <form onSubmit={onSubmit} className="w-full max-w-sm space-y-5">
          ...
          <div className="space-y-1">
            ...
            <input type="email" required className="input" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" placeholder="you@example.com" />
          </div>
          <div className="space-y-1">
          ...
          <button type="submit" className="btn-gradient w-full" disabled={loading}>{loading ? 'Signing in...' : 'Sign in'}</button>
          <p className="text-center text-sm text-slate-500">No account?{' '}
            <Link href="/register" className="font-semibold text-brand hover:underline">Create your workspace</Link>
          </p>
        </form>
      </section>
    </main>
